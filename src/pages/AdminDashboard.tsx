...
const AdminDashboard = () => {
  ...
  const addNoteToLead = async (id: string, note: string) => {
    ...
    try {
      await db.addNoteToLead(id, newNotes);
  ...
  const handleChangePassword = async () => {
    toast({ title: "Information", description: "Veuillez modifier le mot de passe dans votre fichier .env (VITE_ADMIN_PASSWORD)" });
  ...
  const handleSaveOffer = async () => {
    ...
    try {
      if (editingOffer) {
        ...
        await db.insertOffer(offerData);
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    toast({ title: "Note", description: "Pour garder vos images, veuillez héberger vos fichiers (ex: Cloudinary, PostImages) et coller les URLs. L'upload direct nécessite un service de storage externe." });
  };
  ...
  const logout = () => {
    localStorage.removeItem("benak_admin_session");
    navigate("/admin");
  };
