...
// Static admin credentials for independence from external Auth services
// Use VITE_ADMIN_EMAIL and VITE_ADMIN_PASSWORD in your .env
const ADMIN_EMAIL = import.meta.env.VITE_ADMIN_EMAIL || "bena@hills.com";
const ADMIN_PASSWORD = import.meta.env.VITE_ADMIN_PASSWORD || "Azerty2026";
...
const AdminLogin = () => {
  ...
  useEffect(() => {
    // Basic session persistence via localStorage for the standalone project
    const session = localStorage.getItem("benak_admin_session");
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Using localized credentials to remove all traces of Lovable project dependency
    if (email === ADMIN_EMAIL && password === ADMIN_PASSWORD) {
      localStorage.setItem("benak_admin_session", "true");
